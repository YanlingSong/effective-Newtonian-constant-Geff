...
#the dimension of the code is accordant with PINOCCHIO-4.1 from the MG branch of https://github.com/pigimonaco/Pinocchio in the file cosmo.c 
#and you even can replace the mu() function in the PINOCCHIO-4.1 with this mu_CGEFT(a,k)
...
math.pow(H_over_c/H,4.)*OmegaLambda)
                                                         #these function you can refer to Baojiu Li, arXiv:1406.0485 1302.6241 1208.0600
                                                         #and you must remember that diffetent papers have different customs of the Lagrangian,the parameters ci
                                                         #are a little different,now we use the custom from Baojiu Li
    ...
    dM_hatdt=0.
    dM3_bardt=0.
    pdM_hat2dt=0.                                      #refer to arXiv:1304.6712, and don't forget Baojiu Li !!
    pdM1_bar3dt=0.
    dR0dt=0.
...
9.*dHdt*dHdt*M2_bar*M2_bar/2.+3.*dHdt*dHdt*M3_bar*M3_bar/2.  
  ...
  # /* pdM_hat2dt is the partial derivative of M_hat^2 with respect to t, pdM1_bar3dt is the partial derivative of M1_bar^3 with respect to t       */
    ...
    f1=B2*C3-C1*B3
    f2=B2*CPI
    f3=A1*(B3*C2-B1*C3)+A2*(B1*C1-B2*C2)+A3*(B2*C3-B3*C1)                #refer to arXiv:1606.05339 Appendix A
    f4=(A3*B2-A1*B1)*CPI
